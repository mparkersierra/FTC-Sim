...
try {
    ...
    Invoke-Checked $Vcpkg @('install', "opencascade[core,rapidjson,tbb]:$Triplet", "--overlay-triplets=$OverlayTriplets", "--host-triplet=$Triplet", "--x-install-root=$InstalledDir")
    Invoke-Checked 'cmake' @('-S', 'native/cad-step-to-glb', '-B', $BuildDir,
        ...
        "-DVCPKG_OVERLAY_TRIPLETS=$OverlayTriplets", '-DCMAKE_CONFIGURATION_TYPES=Release', "-DVCPKG_INSTALLED_DIR=$InstalledDir")
    ...
    Copy-Item (Join-Path $InstalledDir "$Triplet/bin/*.dll") $NativeBin
    Copy-Item (Join-Path $InstalledDir "$Triplet/share") (Join-Path $NativeRoot 'licenses') -Recurse
